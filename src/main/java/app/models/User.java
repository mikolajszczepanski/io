...
public class User {
	...
	public static List<String> Validate(String username,String password,String email,String answer) {
		...
		if (username == null || username.length() < 5 || username.length() > 20) {
			errors.add("Nazwa użytkownika nie mieści się w przedziale [5, 20].");
		}
		...
		if (email == null || email.length() < 5 || email.length() > 40) {
			errors.add("Email nie mieści się w przedziale [5, 40].");
		}
		...
		if (answer == null || answer.length() < 2 || answer.length() > 40) {
			errors.add("Odpowiedź nie mieści się w przedziale [2, 40].");
		}
		return errors;
	}
